...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Refresh analysis every 20 seconds (includes candle data)
  ...
  setInterval(fetchAutomationStatus, 5000); // Fast update for automation stats
  
  fetchSignalHistory();
  setInterval(fetchSignalHistory, 5000); // Refresh history list

  fetchFunds();
  setInterval(fetchFunds, 10000); // Refresh account funds & P&L
...
async function fetchAutomationStatus() {
  try {
    ...
    pnlEl.className = pnl > 0 ? 'profit' : pnl < 0 ? 'loss' : '';
...
async function triggerLogin() {
  showToast('Initiating Fyers Login...', 'info');
  const resp = await fetch('/api/login');
  const data = await resp.json();
  if (data.success) {
    showToast('Login process started. Check your browser.', 'success');
  } else {
    showToast('Login request failed: ' + data.message, 'error');
  }
}
...
function connectWebSocket() {
  ...
  wsConnection.onopen = () => {
    ...
    document.getElementById('statusBadge').innerHTML = `<span class="status-dot connected"></span>Live`;
  };
    ...
    switch (data.type) {
      ...
      case 'positions':
        ...
        if (pnlEl) {
          ...
          pnlEl.className = 'fund-value ' + (pnl >= 0 ? 'pnl-positive' : 'pnl-negative');
        }
        break;
      case 'orders':
      ...
      case 'strike_update':
        ...
        if (analysisData) {
          // If the top signal is CALL use ce_strikes, else pe_strikes
          let currentTopSignal = analysisData.signals?.[0];
          if (currentTopSignal) {
          ...
          // Re-render signals so their LTP info updates
  ...
  wsConnection.onclose = () => {
    ...
    document.getElementById('statusBadge').innerHTML = `<span class="status-dot" style="background:#ef4444"></span>Reconnecting...`;
...
function updateFundsLive(funds, pnl) {
  if (funds) {
    document.getElementById('fundAvailable').textContent = '₹' + Number(funds.equityAmount || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 });
    document.getElementById('fundUsed').textContent = '₹' + Number(funds.utilisedAmount || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 });
  }
  if (pnl !== undefined) {
...
function updatePnlDisplays(pnl) {
  ...
  if (pnlEl) {
    ...
    pnlEl.style.color = ''; // Clear any inline override
  }
  if (headerPnlEl) {
    headerPnlEl.textContent = `P&L: ${formatted}`;
    headerPnlEl.className = colorClass;
    headerPnlEl.style.color = ''; // Clear any inline override
  }
}
...
function handleVisibilityChange() {
  ...
  // Re-fit content to ensure chart doesn't get stuck in weird zoom/scroll states
  if (chart5m) chart5m.timeScale().fitContent();
...
async function fetchAnalysis() {
  ...
  renderTrend(analysisData.trend);
  ...
  document.getElementById('obCount').textContent = `${analysisData.active_order_blocks.length} OB`;
}
...
async function fetchFunds() {
  try {
    ...
    updateFundsLive(fundData.funds || {}, posData.overall?.total_pnl || 0);
  } catch (e) {
...
function renderOrderBlocksOnChart(obs) {
  ...
  // Show all active OBs + any mitigated OBs from today
  const todayStart = new Date();
  todayStart.setHours(0, 0, 0, 0);
  const todayStartSec = Math.floor(todayStart.getTime() / 1000);
  
  const displayOBs = obs.filter(ob => ob.active || ob.timestamp > todayStartSec);
  const endTime = lastCandleTime5m || Math.floor(Date.now() / 1000) + IST_OFFSET;
  ...
  displayOBs.forEach(ob => {
    ...
    // Use blue color, but faded if mitigated
    const fillAlpha = ob.active ? '0.18' : '0.05';
    const lineAlpha = ob.active ? '0.6' : '0.2';
    const blueFill = `rgba(59, 130, 246, ${fillAlpha})`;
    const blueLine = `rgba(59, 130, 246, ${lineAlpha})`;
    ...
    try {
      ...
      const zoneSeries = chart5m.addBaselineSeries({
        ...
        topFillColor1: blueFill,
        topFillColor2: blueFill,
        topLineColor: blueLine,
      ...
      const borderSeries = chart5m.addLineSeries({
        color: `rgba(59, 130, 246, ${ob.active ? '0.5' : '0.1'})`,
        lineWidth: 1,
        lineStyle: 2,
        lastValueVisible: false,
        priceLineVisible: false,
  ...
  const chartMarkers = displayOBs.map(ob => ({
    ...
    color: ob.active ? '#3b82f6' : '#1e3a8a', // Bright blue if active, dark blue if mitigated
    shape: ob.direction === 'BULLISH' ? 'arrowUp' : 'arrowDown',
    text: ob.active ? 'OB' : 'OB (Mitigated)',
  }));
...
function renderSignals(signals) {
  ...
  container.innerHTML = signals.map((sig, i) => {
    ...
    // Prices: Buy, SL, Target (Estimated for the best strike)
    let priceInfo = '';
    if (bestStrike && !isAdvisory) {
      const buyPrice = bestStrike.ltp || 0;
      const slPoints = 12.0; // Default estimate for UI
      const slPrice = buyPrice - slPoints;
      const tgtPrice = buyPrice + (slPoints * 2.0);
      
      priceInfo = `
        <div class="signal-price-grid">
          <div class="signal-price-item">
            <div class="signal-price-label">BUY PRICE</div>
            <div class="signal-price-value" style="color:var(--accent-cyan)">₹${buyPrice.toFixed(2)}</div>
          </div>
          <div class="signal-price-item">
            <div class="signal-price-label">STOPLOSS</div>
            <div class="signal-price-value" style="color:var(--bearish)">₹${slPrice.toFixed(2)}</div>
          </div>
          <div class="signal-price-item">
            <div class="signal-price-label">TARGET</div>
            <div class="signal-price-value" style="color:var(--bullish)">₹${tgtPrice.toFixed(2)}</div>
          </div>
        </div>
      `;
    }
...
function skipSignal(idx) {
  const cards = document.querySelectorAll('.signal-card');
  if (cards[idx]) {
    cards[idx].style.opacity = '0.3';
    cards[idx].style.pointerEvents = 'none';
  }
  showToast('Signal skipped', 'info');
}
    ...
    html += `
      <div class="history-item">
        ...
        <div class="history-main">
          <span class="history-type" style="color: ${item.type==='BUY' ? 'var(--bullish)' : 'var(--bearish)'}">${item.type} NIFTY</span>
